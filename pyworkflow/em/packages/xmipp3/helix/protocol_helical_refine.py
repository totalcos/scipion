...
class XmippProtHelixRefine(XmippProtProjMatch):
    """    
    Reconstruct an helical initial tube from 2d classification.
    """
    _label = 'helical refine'
    ...
    def _defineParams(self, form):
        ...
        #SelFileName
        form.addParam('useInitialAngles', params.BooleanParam, default=False,
                     label="Use initial angles/shifts ? ",
                     help='Set to *Yes* if you want to use the projection \n'
                          'assignment (angles/shifts) associated with the \n'
                          'input particles (hasProjectionAssigment=True)')
        # ReferenceFileNames
        form.addParam('input3DReferences', params.PointerParam,
                     pointerClass='Volume',
                     label='Initial 3D reference volume',
                     help='Input 3D reference reconstruction.')

        self.cleanUpFiles = pwobj.Boolean(False)

        # CTF
        self.doCTFCorrection = pwobj.Boolean(True)
        self.doAutoCTFGroup = pwobj.Boolean(True)
        self.ctfGroupMaxDiff = pwobj.Float(0.1)
        self.ctfGroupMaxResol = pwobj.Float(5.6)
        self.setOfDefocus = pwobj.String('')
        self.paddingFactor = pwobj.Float(2)
        self.wienerConstant = pwobj.Float(-1)

        # Mask
        self.maskType = pwobj.Integer(0) # None
        form.addHidden('maskType', params.EnumParam,
                       choices=['None', 'circular', 'mask object'],
                       default=xmipp3.MASK2D_CIRCULAR,
                       display=params.EnumParam.DISPLAY_COMBO,
                       label="Mask reference volumes")

        self.maskRadius = pwobj.Integer(-1)
        self.inputMask = pwobj.Pointer() # VolumeMask

        form.addSection(label='Helical Projection Matching')

        form.addParam('numberOfIterations', params.IntParam, default=4,
                      label='Number of iterations',
                      help='Number of iterations to perform.')
        form.addHidden('innerRadius', params.NumericListParam, default='0',
    ...
    *Note:* if there are more values than iterations the extra value are ignored
    """)
        ...
        form.addHidden('outerRadius', params.NumericListParam, default='64',
    ...
    *Note:* if there are more values than iterations the extra value are ignored
    """)
        ...
        form.addHidden('availableMemory', params.IntParam, default=2,
    ...
    """)
        ...
        form.addHidden('angSamplingRateDeg', params.NumericListParam, default='7 5 3 2',
    ...
    *Note:* if there are more values than iterations the extra value are ignored
    """)
        ...
        form.addParam('maxChangeInAngles', params.NumericListParam, default='1000 10 4 2',
    ...
    """)
        ...
        form.addParam('perturbProjectionDirections', params.NumericListParam, default='0',
                 label='Perturb projection directions?', expertLevel=params.LEVEL_ADVANCED,
    ...
    *Note:* if there are more values than iterations the extra value are ignored
    """)
        ...
        form.addParam('projectionMethod', params.EnumParam, choices=['fourier', 'real_space'],
                 default=xmipp3.PROJECT_REALSPACE, expertLevel=params.LEVEL_ADVANCED,
                 label="Projection method", display=params.EnumParam.DISPLAY_COMBO,
                 help='select projection method, by default Fourier with padding 1 and interpolation bspline')


        form.addParam('paddingAngularProjection', params.FloatParam, default=1, expertLevel=params.LEVEL_ADVANCED,
                 condition='projectionMethod == %d' % xmipp3.PROJECT_FOURIER,
                 label='Padding factor for projection', validators=[params.GE(1)],
    ...
    """)
    # Changed from String to Int
        form.addParam('kernelAngularProjection', params.EnumParam, choices=['neareast', 'linear', 'bspline'],
                 default=xmipp3.KERNEL_BSPLINE, expertLevel=params.LEVEL_ADVANCED,
                 condition='projectionMethod == %d' % xmipp3.PROJECT_FOURIER,
                 label='Interpolation kernel for projection',
                 help=""" Interpolation kernel for the generation of projections.
    """)

        form.addParam('maxChangeOffset', params.NumericListParam, default='1000 10 5',
                 label='Maximum change in origin offset', expertLevel=params.LEVEL_ADVANCED,
                 help=""" If set to 1, this option will result to a Gaussian perturbation to the
    evenly sampled projection directions of the reference library.
    This may serve to decrease the effects of model bias.
    You may specify this option for each iteration.
    ...
    *Note:* if there are more values than iterations the extra value are ignored
    """)
        ...
        form.addParam('search5DShift', params.NumericListParam, default='4x5 0',
    ...
    *Note:* if there are more values than iterations the extra value are ignored
    ...
    """)
        form.addParam('search5DStep', params.NumericListParam, default='2',
                 label='Step size for 5D translational search', expertLevel=params.LEVEL_ADVANCED,
    ...
    *Note:* if there are more values than iterations the extra value are ignored
    """)
        ...
        form.addParam('doRestricSearchbyTiltAngle', params.BooleanParam, default=False, expertLevel=params.LEVEL_ADVANCED,
                 label="Restrict tilt angle search?",
                 help ='Restrict tilt angle search \n ')

        form.addParam('tilt0', params.FloatParam, default=-91., condition='doRestricSearchbyTiltAngle',
                 label="Lower-value for restricted tilt angle search",
                 help ='Lower-value for restricted tilt angle search \n ')

        form.addParam('tiltF', params.FloatParam, default=-91., condition='doRestricSearchbyTiltAngle',
    ...
    """)
        form.addParam('symmetryGroupNeighbourhood', params.TextParam, default='', expertLevel=params.LEVEL_ADVANCED,
    ...
    """
        ...
        form.addParam('onlyWinner', params.NumericListParam, default='0',
                 label='compute only closest neighbor', expertLevel=params.LEVEL_ADVANCED,
    ...
    *Note:* if there are more values than iterations the extra value are ignored
    """)
        ...
        form.addParam('discardImages', params.EnumParam,
                 choices=['None', 'maxCC', 'percentage', 'classPercentage'],
                 default=xmipp3.SELECT_NONE, display=params.EnumParam.DISPLAY_COMBO,
    ...
    """)
        form.addParam('minimumCrossCorrelation', params.NumericListParam, default='0.1',
    ...
    *Note:* if there are more values than iterations the extra value are ignored
    """)
        ...
        form.addParam('discardPercentage', params.NumericListParam, default='10',
    ...
    *Note:* if there are more values than iterations the extra value are ignored
    ...
    """)
        ...
        form.addParam('discardPercentagePerClass', params.NumericListParam, default='10',
    ...
    *Note:* if there are more values than iterations the extra value are ignored
    ...
    """)
        ...
        form.addParam('doScale', params.BooleanParam, default=False,
                 label="Perform scale search?",  expertLevel=params.LEVEL_ADVANCED,
                 help=' If true perform scale refinement. (UNDER DEVELOPMENT!!!!) \n  ')

        form.addParam('scaleStep', params.NumericListParam, default=1, condition='doScale',
    ...
    *Note:* if there are more values than iterations the extra value are ignored
    Set to zero to prevent discarding any images''')
        ...
        form.addParam('scaleNumberOfSteps', params.NumericListParam, default=3, condition='doScale',
    ...
    """)
        ...
        form.addParam('projMatchingExtra', params.StringParam, default='',
                 label='Additional options for Projection_Matching', expertLevel=params.LEVEL_ADVANCED,
    ...
    """)
        ...
        # 2D re-alignment of classes
        self.performAlign2D = pwobj.Boolean(False)
        self.doAlign2D = pwobj.String('0')
        self.align2DIterNr = pwobj.String('4')
        self.align2dMaxChangeOffset = pwobj.String('2x1000 2x10')
        self.align2dMaxChangeRot = pwobj.String('2x1000 2x20')


        form.addSection(label='3D Reconstruction')

        form.addParam('reconstructionMethod', params.EnumParam, expertLevel=params.LEVEL_ADVANCED,
                 choices=['fourier', 'art', 'wbp'],
                 default=xmipp3.RECONSTRUCT_FOURIER, display=params.EnumParam.DISPLAY_COMBO,
    ...
    """)
        ...
        form.addParam('fourierMaxFrequencyOfInterest', params.DigFreqParam, default=0.25,
                 condition='reconstructionMethod == %d' % xmipp3.RECONSTRUCT_FOURIER,
                 label='Initial maximum frequency', expertLevel=params.LEVEL_ADVANCED,
    ...
    """)
        form.addParam('fourierReconstructionExtraCommand', params.StringParam, default='',
                 condition='reconstructionMethod == %d' % xmipp3.RECONSTRUCT_FOURIER,
                 label='Additional parameters for fourier', expertLevel=params.LEVEL_ADVANCED,
                 help=""" For details see:
    [[http://xmipp.cnb.csic.es/twiki/bin/view/Xmipp/Fourier][fourier]]
    """)

        form.addParam('artLambda', params.NumericListParam, default='0.2',
                 condition='reconstructionMethod == %d' % xmipp3.RECONSTRUCT_ART,
                 label='Values of lambda for ART', expertLevel=params.LEVEL_ADVANCED,
    ...
    *Note:* if there are more values than iterations the extra value are ignored
    """)
        ...
        form.addParam('artReconstructionExtraCommand', params.StringParam, default='-k 0.5 -n 10 ',
                 condition='reconstructionMethod == %d' % xmipp3.RECONSTRUCT_ART,
                 label='Additional parameters for ART', expertLevel=params.LEVEL_ADVANCED,
                 help=""" For details see:
    [[http://xmipp.cnb.csic.es/twiki/bin/view/Xmipp/Art][xmipp art]]
    """)

        form.addParam('wbpReconstructionExtraCommand', params.StringParam, default='',
                 condition='reconstructionMethod == %d' % xmipp3.RECONSTRUCT_WBP,
                 label='Additional parameters for WBP', expertLevel=params.LEVEL_ADVANCED,
                 help=""" For details see:
    [[http://xmipp.cnb.csic.es/twiki/bin/view/Xmipp/Wbp][xmipp wbp]]
    """)

        form.addParam('doComputeResolution', params.BooleanParam, default=True,
                 label='Compute resolution?', expertLevel=params.LEVEL_ADVANCED,
                 help=""" For details see:
    [[http://xmipp.cnb.uam.es/twiki/bin/view/Xmipp/Resolution][xmipp resolution]].
    """)

        form.addParam('doSplitReferenceImages', params.NumericListParam, default='1',
                 label='Split references averages?', expertLevel=params.LEVEL_ADVANCED,
    ...
    *Note:* if there are more vapplications/scripts/protocols/new_protocol_projmatch.pyalues than iterations the extra value are ignored
    """)
        ...
        form.addParam('doLowPassFilter', params.BooleanParam, default=True,
                 label="Low-pass filter the reference?")

        form.addParam('useFscForFilter', params.BooleanParam, default=True,
   ...
   i.e. pixel^-1: minimum 0, maximum 0.5)
    """)
        ...
        form.addParam('constantToAddToFiltration', params.NumericListParam, default='0.05',
    ...
    *Note:* if there are more values than iterations the extra value are ignored
    """)
        ...
        form.addParam('constantToAddToMaxReconstructionFrequency', params.NumericListParam, default='0.1',
                 label='Constant to be added to the reconstruction maximum frequency', expertLevel=params.LEVEL_ADVANCED,
                 condition="doLowPassFilter",
    ...
    *Note:* if there are more values than iterations the extra value are ignored
    """)
        ...
        form.addParam('mpiJobSize', params.IntParam, default=2,
                  label='MPI job size', expertLevel=params.LEVEL_ADVANCED,
